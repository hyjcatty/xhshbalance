...
switch ($key){
    ...
    case "XH_Balance_Run": //Use Wechat to login the Server, response is the userID in system.
        ...
        * */
            ...
            $client_id = "MQTT_XH_High_Simple_Balance_PHP"; // make sure this is unique for connecting to sever - you could use uniqid()
            $mqtt = new phpMQTT($server, $port, $client_id);
            if(!$mqtt->connect(true, NULL, $username, $password)) {
                exit(1);
            }
            $topics['MQTT_XH_High_Simple_Balance_PHP'] = array("qos" => 0, "function" => "procmsg");
            //$mqtt->subscribe($topics, 0);
            $retval;
            if($action == "start"){
                $retval=array(
                    'action'=>'XH_High_Simple_Balance_config_start'
                );
            }else{
                $retval=array(
                    'action'=>'XH_High_Simple_Balance_config_stop'
                );
            }
            $mqtt->publish('MQTT_XH_High_Simple_Balance_HCU', _encode($retval));
    ...
    case "XH_Balance_Pause": //Use Wechat to login the Server, response is the userID in system.
        ...
        * */
            ...
            if($action == "pause"){
                $retval=array(
                    'action'=>'XH_High_Simple_Balance_config_pause'
                );
            }else{
                $retval=array(
                    'action'=>'XH_High_Simple_Balance_config_resume'
                );
            }
...
function calizero(){
    ...
    //$mqtt->subscribe($topics, 0);
    $retval=array(
                       'action'=>'XH_High_Simple_Balance_calibration_zero_trigger'
                   );
...
function caliweight(){
    ...
    //$mqtt->subscribe($topics, 0);
    $retval=array(
                       'action'=>'XH_High_Simple_Balance_calibration_weight_trigger'
                   );
...
function calidynamic($bool){
    ...
    $topics['MQTT_XH_High_Simple_Balance_PHP'] = array("qos" => 0, "function" => "procmsg");
    //$mqtt->subscribe($topics, 0);
    $action = 'XH_High_Simple_Balance_calibration_dynamic_stop';
    if($bool) $action = 'XH_High_Simple_Balance_calibration_dynamic_start';
    $retval=array(
                       'action'=>$action
                   );
...
function flushUI(){
    ...
    //$mqtt->subscribe($topics, 0);
    $retval=array(
                       'action'=>'XH_High_Simple_Balance_force_flush'
                   );
...
function runpause(){
     ...
     //$mqtt->subscribe($topics, 0);
     $retval=array(
                        'action'=>'XH_High_Simple_Balance_pause_trigger'
                    );
...
function runresume(){
    ...
    //$mqtt->subscribe($topics, 0);
    $retval=array(
                       'action'=>'XH_High_Simple_Balance_resume_trigger'
                   );
